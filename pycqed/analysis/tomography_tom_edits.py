...
class TomoAnalysis_JointRO():
    ...
    def __init__(self, measurements_cal, measurements_tomo,
        ...
        self.err_cal = err_cal
        self.err_tomo = err_tomo
    ...
    def execute_pseudo_inverse_tomo(self):
        ...
        data = self._calculate_coefficient_matrix()
        ...
        # first skip beta0
        reduced_inverse = np.linalg.pinv(coefficient_matrix[:, 1:])
        basis_decomposition[1:] = reduced_inverse @ self.measurements_tomo
        # re-add beta0
        basis_decomposition[0] = 1

        if self.err_cal is not None:
            a = ((reduced_inverse**2) @ (self.err_tomo**2) +
                 (reduced_inverse**2) @ (err_coefficient_matrix[:, 1:]**2) @
                 (reduced_inverse**2) @ (self.measurements_tomo**2))

            err_decomposition[1:] = np.sqrt(a)
            err_decomposition[0] = 0
        ...
        if self.err_cal is not None:
            print(self.err_cal)
            return basis_decomposition, err_decomposition, rho
        else:
...
##############################################################
    ...
    def _calibrate_betas(self, measurements_cal, err_cal=None):
        """
    ...
    def _calculate_coefficient_matrix(self):
        ...
        coefficient_matrix = np.zeros(
            (self.n_quadratures * len(self.rotation_matrixes) ** self.n_qubits,
             4 ** self.n_qubits))
        if self.err_cal is not None:
            err_coefficient_matrix = np.zeros(
                (self.n_quadratures *
                 len(self.rotation_matrixes) ** self.n_qubits,
                 4 ** self.n_qubits))
        ...
        for quadrature in range(self.n_quadratures):
            ...
            if self.err_cal is not None:
                self.betas, self.err_betas = self._calibrate_betas(
                    self.measurements_cal[
                        quadrature * self.n_states:
                        (1 + quadrature) * self.n_states],
                    self.err_cal[
                        quadrature * self.n_states:
                        (1 + quadrature) * self.n_states])
...
def plot_operators(results, ax, labels=None, err_cal=False):
    # NB: reorders the pauli expectation values to correct for convention
    pauli_1, pauli_2, pauli_cor = order_pauli_output2(results)
    if err_cal is not False:
        err_pauli_1, err_pauli_2, err_pauli_cor = order_pauli_output2(err_cal)
    width = 0.35
    ind1 = np.arange(3)
    ind2 = np.arange(3, 6)
    ind3 = np.arange(6, 15)
    if err_cal is not False:
        ax.bar(ind1, pauli_1, width, yerr=err_pauli_1, color='r', align='center')
        ax.bar(ind2, pauli_2, width, yerr=err_pauli_2, color='b', align='center')
        ax.bar(ind3, pauli_cor, width, yerr=err_pauli_cor, color='purple', align='center')
    else:
...
class Tomo_Multiplexed(ma.MeasurementAnalysis):
    ...
    def run_default_analysis(self, num_measurements=1e4, **kw):
        ...
        err_mean_h_list = [mean_h * (1 - mean_h) / np.sqrt(num_measurements*28)
                           for mean_h in [mean_h1, mean_h2, mean_h12]]
        # print('########### OFFSETS ###########')
        # print('<H1>',mean_h1)
        # print('<H2>',mean_h2)
        # print('<H12>',mean_h12)
        ...
        avg_h2 -= mean_h2
        avg_h12 -= mean_h12
        err_avg_h_list = [np.sqrt(err_avg**2 + err_trace**2)
                          for err_avg, err_trace
                          in zip(err_avg_h_list, err_mean_h_list)]
        ...
        err_scale_h_list = [scale_h * (1 - scale_h) /
                            ...
                            for scale_h in scale_h_list]
        # print('########### SCALES ###########')
        # print('H1',scale_h1)
        # print('H2',scale_h2)
        # print('H12',scale_h12)

        err_avg_h_list = [np.abs(avg_h/scale_h) * np.sqrt(err_scale_h**2 / scale_h**2 +
                                                  err_avg_h**2 / avg_h**2)
                          for avg_h, scale_h, err_avg_h, err_scale_h in zip(
                            avg_h_list, scale_h_list,
                            err_avg_h_list, err_scale_h_list)]
        ...
        # This is an absolute scaling, so I think it just scales the error?
        std_h_list = [std_h1, std_h2, std_h12]
        ...
        err_tomo = (np.array([err_avg_h[0:36]
                              for err_avg_h in err_avg_h_list])).flatten()
        ...
        err_cal = np.array([
            np.mean(err_avg_h[36+7*j:36+7*(j+1)])/np.sqrt(7)
            for err_avg_h in err_avg_h_list for j in range(4)])
        ...
        tomo = TomoAnalysis_JointRO(
            measurements_cal, measurements_tomo,
    ...
    def plot_LI(self):
        ...
        if self.op_err is not None:
            plot_operators(self.operators, ax, labels=self.meas_op_labels, err_cal=self.op_err)
        else:
